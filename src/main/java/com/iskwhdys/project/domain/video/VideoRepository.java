...
public interface VideoRepository
  ...
  List<VideoEntity> findByTypeInAndEnabledTrueAndIdNotInOrderByLiveStartDesc(List<String> type,
      List<String> id);

  List<VideoEntity> findByTypeInAndEnabledTrueOrderByLiveStartDesc(List<String> type);


  @Query(value = "select * from public.videos " + "where" + "	id not in ?1 and"
  ...
  List<VideoEntity> findByEnabledTrueAndTypeEqualsAndLiveStartBetweenOrderByLiveStartDesc(String of,
      Date start, Date end);
  ...
  List<VideoEntity> findTop10ByEnabledTrueAndChannelIdEqualsAndUploadDateBeforeOrderByUploadDateDesc(
      String of, Date from);
